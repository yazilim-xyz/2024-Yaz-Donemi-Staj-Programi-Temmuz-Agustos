...
const CategoryList = ({ setSelectedCategory, darkMode, isTabBar = false }) => {
  ...
  if (isTabBar) {
    return (
      <div className={`flex justify-between ${darkMode ? 'bg-gray-800' : 'bg-gray-100'} p-2 rounded-t-lg shadow-md overflow-auto`}>
        ...
        <div className="flex overflow-x-auto scrollbar space-x-2 pb-2">
          {categories.map((category) => (
            <button
              key={category.id}
              onClick={() => setSelectedCategory(category.name)}
              className={`text-sm p-2 flex-1 rounded-lg whitespace-nowrap ${darkMode ? 'bg-gray-700 text-white' : 'bg-white text-black'} hover:bg-primary hover:text-white transition-all`}
            >
              {category.name}
            </button>
          ))}
