...
const ProductList = ({ darkMode }) => {
  ...
  return (
    <div className={`p-4 rounded-3xl shadow max-h-[calc(100vh-6rem)] overflow-y-auto ${darkMode ? 'bg-primary' : 'bg-white'}`}>
      <h2 className={`text-xl font-bold mb-4 ${darkMode ? 'text-white' : 'text-black'}`}>Kategoriler</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
        {products.map((product, index) => (
          <div key={index} className={`border p-4 rounded-3xl shadow-sm ${darkMode ? 'bg-white text-black' : 'bg-primary text-white'}`}>
