...
const Navbar = () => {
  ...
  const [menuOpen,setMenuOpen] = useState(false); // başlangıçta menü kapalı
  ...
  return (
    <div className='flex items-center justify-between bg-[#3955b0] h-[60px] px-4 sm:px-6 md:px-10 lg:px-12  text-white shadow-lg rounded'>
      ...
      <div className='flex items-center gap-2 sm:gap-4 relative '>
        ...
        >
          <FaUsers 
          style={{ color: 'white', fontSize: '20px', sm: '24px' }} 
          className="cursor-pointer hover:text-white transition duration-300" 
          
        />
        <FaChevronDown
          style={{fontSize:'20px',sm:'24px'}}
          className={`ml-1 text-gray-300 text-sm transition-transform duration-300 ${menuOpen ?'rotate-180' : ''}` }
        />
        </div>
        {menuOpen && (
